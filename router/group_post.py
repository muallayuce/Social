...
@router.get('/{group_id}/{post_id}', response_model=GroupPostDisplay)
def get_group_post(group_id: int, post_id: int, user_id: int = Query(...), db: Session = Depends(get_db)):
...
@router.put('/{group_id}/posts/{post_id}')
